...
public class EmotionDetector extends Activity implements Detector.ImageListener, CameraDetector.CameraEventListener {
    ...
    public void onImageResults(List<Face> list, Frame frame, float v) {
        ...
        if (list.size() == 0) {
            ...
            int Sadness_score = (int) (face.emotions.getSadness());
            int happiness_score = (int) (face.expressions.getSmile());

            //     smileTextView.setText(String.format("Emotions\n%d",happiness_score));


            if (happiness_score == Sadness_score) {
                smileTextView.setText(String.format("Emotions\n%s", "Neutral"));
                HoldMood holdMood=new HoldMood("Neutral");

                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        startActivity(new Intent(EmotionDetector.this, Neutral.class));
                    }
                }, 3000);

            } else if (Sadness_score > happiness_score) {
                smileTextView.setText(String.format("Emotion\n%s", "Sad"));
                HoldMood holdMood = new HoldMood("Sad");
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        startActivity(new Intent(EmotionDetector.this, Sad.class));
                    }
                }, 3000);

            } else if (happiness_score > Sadness_score) {
                smileTextView.setText(String.format("Emotions\n%s", "Happy"));
                HoldMood holdMood = new HoldMood("Happy");
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        startActivity(new Intent(EmotionDetector.this, Happy.class));
                    }
                }, 3000);
            }
...
/*
    ...
    public void nextActivity(String mood){
        //  startActivity(new Intent(EmotionDetector.this,PlayActivity.class));
    }
}
